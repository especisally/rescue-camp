...
- 修复器材装备页面不显示数据 Bug（`filteredList` → `equipments` 变量名不匹配）
- 删除报修审批/领用审批全链路代码（前后端路由 + 视图 + API + 侧边栏导航，功能不再需要）
- 后台管理表单 URL 字段增加"上传"按钮（视频/训练/学习/装备/Banner 共 5 个表单 9 个字段）
- 视频上传后自动读取时长并填充 duration 字段
- 修复视频播放页完全硬编码问题，改为动态数据绑定
- 版本号 0.2.3 → 0.2.4
- 详细日志见：`feature-upload-and-fixes.md`
